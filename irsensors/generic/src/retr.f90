...
!*************************************************************</f90Main>
  ...
  USE IRReadStdInputs
  USE InvertModule
  use constants, ONLY : MISSING_REAL,MISSING_CHAR,MISSING_INT
  ...
  real,    dimension(mxlev)              :: PrefOSS ! From OSS file, has to be dimensioned here.
  ...
  !----IR-related arrays
  REAL,    DIMENSION(:), pointer         :: frqEmIR
  ...
  integer            :: ih2o,i,ii,ifor
  integer            :: time(6)=MISSING_INT
  ...
  call readStd(nprofs,debug,mwCld,iCell,icasc, &
       iembkgflg=iembkgflg,iextbkg=iextbkg, &
       kchanOut=kchan,DynamicNoise=DynamicNoise, &
  ...
  CALL IRbkgPrepInit(nparG,IG,NG,nlev,pref,nchanmw,Frq,pol, &
  ...
  if (enableScatter) then
     !initialize the cldScene for scattering. npProperty has to be
     !consistent with the number of elements in the propertyNames.
     allocate(cldScene(nHyd))
     npProperty = 3
     allocate(propertyNames(npProperty))
     propertyNames(1:npProperty) = (/'SizP', 'SizS', 'Temp'/)  !version
     do iHydr = 1, nHyd
        cldScene(iHydr)%name = dataType_cld(iHydr)
        cldScene(iHydr)%nLevel = nLev
        cldScene(iHydr)%nLayer = nLev - 1
        allocate(cldScene(iHydr)%physProp(nLev))
        allocate(cldScene(iHydr)%hydrAmt(nLev))
        do ilev = 1, nLev
           cldScene(iHydr)%physProp(ilev)%nProperty = npProperty
           allocate(cldScene(iHydr)%physProp(ilev)%property(npProperty))
           do ipp = 1, npProperty
              cldScene(iHydr)%physProp(ilev)%property(ipp)%name = trim(propertyNames(ipp))
              cldScene(iHydr)%physProp(ilev)%property(ipp)%value = 0.0
           end do
        end do
     end do
  end if
  ...
  call oss_init_ir(U_osscoefs_ir,U_ossoptdpth_ir,U_ossScatRegr_ir, &
       ...
       ossScatRegrIRon,enableScatter,dataType_cld,cldTyp,cldScene,nHyd, &
  ...
  !-----------------------------------------------------
  if (MWon) then
     call openScene(ncid=U_retr,file=F_Retr,MolId=molId,pressure=pref, &
          polarity=pol(1:nchanmw),freq=frq(1:nchanmw),casename=F_Retr, &
          nLevel=nlev,nchmw=nchanmw,status='new',IG=IG,NG=NG)
  endif
  if (PHYSon) then
     if (MWon) then
        call openScene(ncid=U_retrIR,file=F_RetrIR,MolId=molId,pressure=pref, &
             freq=frq(1:nchanmw),casename=F_RetrIR, &
     ...
     if (MWon) then
        call openScene(ncid=U_guess,file=F_Guess,MolId=molId,pressure=pref, &
             freq=frq(1:nchanmw),casename=F_Guess, &
             sfcGrid=frqEmIR(1:nemIrG),nSfcGrid=nemIrG, &
             nLevel=nlev,nchmw=nchanmw,status='new',IG=IG,NG=NG)
     else
        call openScene(ncid=U_guess,file=F_Guess,MolId=molId,pressure=pref, &
             casename=F_Guess, &
             sfcGrid=frqEmIR(1:nemIrG),nSfcGrid=nemIrG, &
             nLevel=nlev,nchmw=nchanmw,IG=IG,NG=NG,status='new')
     endif
  endif
  if(REGRESon) then
  ...
  ProfLoop: do ifor=1,nprofs
     ...
     call setSfc(iStrat,iexit,xGesG,igeo,plandAvg(ifor),IG,NG,IEmMwG,nchanmw, &
        plandMaxOc)
...
100  continue
     ...
     if(REGRESon)then
        call classatmRegr(tbEquiv,lat,lon,time,igeo,eia(1),iclassatmRegr)
        call setFOVRegr(iclassatmRegr,igeo,psfc(ifor),ifor,&
     ...
     if(PHYSon) then
        ...
        call classatm(Ym,lat,lon,time,igeo,iclassatm)

        ! Disable retrieval of cloud parameters per cloud mask
        if (flagDynCldMask) call cloudModeTest(cloudType,retrievalFlags)
        ...
        call IRsetFOVbkg(iclassatm,igeo,psfc(ifor),ifor, &
                                 umtx(1:nPar,1:nParG),vmtx(1:nParG,1:nPar))
        if (loadExtDone) call IRsetExtBkg(xbakg_Ext(:,ifor))
        if (loadEmisDone) call IRsetEmisBkg(xbakg_Emis(:,ifor),nemirExt)
        call IRcombBkg(IGExt,NGExt,psfc(ifor),plandAvg(ifor),plandMaxOc, &
             molTran,xbakg,ut_cov_net)
        ...
        if(REGRESon) then
           ...
           call IRmap_retr2geo(xGes(1:nPar),xBakG,xGesG,EmMw,TSfc, &
                     vmtx(1:nParG,1:nPar),nParG,nPar,IG,NG, &
        ...
        if (MWon) then
           call ossdrv_mw(xGesG,EmMw,eia(1),Y,xktG,xkEmMw,kchan,plandAvg(ifor))
           ...
           MwIterLoop: do itermw=1,nxiterMw
              ...
              call IRmap_retr2geo(xGesNew(1:nPar),xBakG,xGesG,EmMw,TSfc, &
                      vmtx(1:nParG,1:nPar),nParG,nPar,IG,NG,pref,nlev, &
              ...
              call chkges(U_algconfig,F_algconfig,xGesG,ifail, &
                   Sx(1:nPar,1:nPar),clw_cov,ciw_cov,nlev,pref, &
              ...
              elseif(chiSqMw < chisqconvMw .and. iStrat == 3)then
                 if(iexit == 0) then
                    ...
                    call setSfc(iStrat,iexit,xGesG,igeo, &
                         plandAvg(ifor),IG,NG,IEmMwG,nchanmw,plandMaxOc)

                    print "('switching to stratified Cov,','igeo = ',i2)",igeo
...
!        ENDIF
        ...
        IRIterLoop: do iter=1,nxiter
           ...
           call set_IRMW_Invert(xGesG,Ym,Y,xktG,xkEmMw, &
                ...
                nchan,IG,NG,IEmMwG,IEmIrG,nemIrG,pref,Airs_Err,logSize)
           ...
           call IRmap_retr2geo(xGesNew(1:nPar),xBakG,xGesG,EmMw,TSfc, &
                vmtx(1:nParG,1:nPar),nParG,nPar,IG,NG,pref,nlev,nchanmw, &
           ...
           call chkges(U_algconfig,F_algconfig, &
                xGesG,ifail,Sx(1:nPar, 1:nPar),clw_cov,ciw_cov,nlev,pref, &
                IG,NG,IR,NR,ih2o,debug,invCldCov_in=.true.)

           if (MWon)call ossdrv_mw(xGesG,EmMw,eia(1),Y,xktG,xkEmMw,kchan,plandAvg(ifor))
           call ossdrv_ir(xgesG(1:npargAtm),EmRf,eia(1),sunang=90.,y=y(nchanmw+1:nchan), &
                xkt=xktg(1:npargAtm,nchanmw+1:nchan),xkEmRf=xkEmRf,pland=plandAvg(ifor))
           ...
           CALL map_geo2retr(xGesG,xBakG,Tsfc,xGes(1:nPar), &
                     umtx(1:nPar,1:nParG),nParG,nPar,IG,NG,nmol,molTran,logSize)
...
!*******************************************************</f90Subroutine>
  ...
  !  Write data for linear inversion to a file
    ...
    call openScene(U_linInvert,fileLinInv,MolId=molId,pressure=pref, &
...
!*******************************************************</f90Subroutine>
  ...
  ! Load external emissivity data for use in background
    ...
    allocate (xbakg_Emis(nemirExt,nforEmis),xEmisTmp(nemirExt),qcEmis(nqcEmis))
    do n=1,nforEmis
       CALL getCov(ncid=U_IRemissExt,irec=n,dmEmIR=xEmisTmp,qc=qcEmis)
       xbakg_Emis(:,n)=xEmisTmp(:)
  ...
  subroutine cloudModeTest(cloudType,retrievalFlags)
...
!*******************************************************</f90Subroutine>
  ...
  SUBROUTINE finiteDiff(xG,EmRf,obsang,sunang,yref,xkt,xkEmRf,pland)
...
!*******************************************************</f90Subroutine>
    ...
    if (NR%cldIce > 0) then
       i=1
       do while( pref(i) < xG(IG%cldIce) )
          i=i+1
          if (i .GE. size(pref)) exit
       enddo
       ilevIce=i
       xGFD(IG%cldIce)=delCldIceTop*(pref(ilevIce)-pref(ilevIce-1))
       xGFD(IG%cldIce+2)=delCldIceAmt
       xGFD(IG%cldIce+3)=delCldIceDeff*xG(IG%cldIce+3)
    endif
    if (NR%cldLiq > 0) then
       i=1
       do while( pref(i) < xG(IG%cldLiq) )
          i=i+1
          if (i .GE. size(pref)) exit
       enddo
       ilevLiq=i
       xGFD(IG%cldLiq)=delCldLiqTop*(pref(ilevLiq)-pref(ilevLiq-1))
    ...
    do i=1,size(xG)
       if(xGFD(i) == 0.) cycle
       xG(i)=xG(i)+xGFD(i)
       call ossdrv_ir(xG,EmRf,obsang,sunang,yFD,xktGFD,xkEmRf,pland)
       xG(i)=xG(i)-xGFD(i)
