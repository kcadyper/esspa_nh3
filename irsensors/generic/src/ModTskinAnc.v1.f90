...
!  Other variables
   ...
   character(len=200)                  :: fIn,fRetv,fOut
   integer                             :: ncidIn
   integer                             :: ncidOut
   integer                             :: ncidRetr
...
!  Namelist
   ...
   namelist /runModL2/ fIn, fRetv, fOut 
...
! Open Scene files
     print *,'fscene ',fIn,"  ",fout
     call openScene(ncidIn,fIn,nprf=nprf,IG=IG,NG=NG,status='old')
...
! For each profile; read in existing state vector and modify SFCT
     do ip=1,nprf
        ...
        call replaceScene(ncidOut,ip,x=xstate)
     enddo
