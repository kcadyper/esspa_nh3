; Plots CrIS NH3, VIIRS derived  cloudy reflectance and CrIS Nh3 with cloudy pixels deleted
...
for ip=0,2 do begin
   ...
   1:begin
	...
	cndex = where(clevels_r gt viirs_refl[id_band,id_viirs_sel,ic],ng)
	if (ng gt 0) then id = cndex[0]-1 else id = 10 
	if (viirs_refl[id_band,id_viirs_sel,ic] gt 0.0) then begin
	   oplot,lon_eta,lat_eta
	   polyfill,lon_eta,lat_eta,color=cores_n[id]
	endif
      endfor
   ...
   2: begin
	 ...
	 for ie=0,neta-1 do begin
        ...
        cndex = where(clevels_n gt cris_sfc_arr[ic],ng)
	if (ng gt 0) then id = cndex[0]-1 else id = 11 
	if (cris_sfc_arr[ic] gt 0.0 AND viirs_refl[id_band,id_viirs_sel,ic] lt 0.3) then begin
	   oplot,lon_eta,lat_eta
	   polyfill,lon_eta,lat_eta,color=cores_n[id]
	endif
      endfor
