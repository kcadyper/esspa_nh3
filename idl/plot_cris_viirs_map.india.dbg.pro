...
for is=0,2 do begin
   device,filename='plot_cris_viirs_map_refl.'+viirs_subsets[is]+'.dbg.ps'
   ...
   for ic=0,ncdf_cris-1 do begin
     ...
     cndex = where(clevels gt viirs_refl[id_band,is,ic],ng)
     if (ng gt 0) then id = cndex[0]-1 else id = 11 
     if (viirs_refl[id_band,is,ic] gt 0.0) then begin
