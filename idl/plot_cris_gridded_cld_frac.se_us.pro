...
if (plot_type eq 'PS') then begin
   ...
   device,filename='plot_cris_gridded_cld_frac.se_us.ps'
   !p.font = 0
endif
...
kount = 0
; Read CrIS data
spawn,'ls '+retr_dir+'retr*se*nc',cdf_files
...
; Read VIIRS  data
   ...
   viirs_cld_frac = float(datav.viirs_count[2,*,*,*])/float(datav.viirs_count[0,*,*,*])
   ...
   if (ic eq 0)  then $
      viirs_cld_frac_arr = reform(viirs_cld_frac,npts_cris) $ 
   else viirs_cld_frac_arr = [viirs_cld_frac_arr,reform(viirs_cld_frac,npts_cris) ]
...
titulo = ['NH3:all','NH3:pol+mod','NH3:all-clouds']
for ip=0,2 do begin
   ...
   for ilat=0,nlat-2 do begin
	 ...
	 if (na gt 0) then begin
	    ...
	    if (no gt 0) then begin
	       case ip of
		  0: vndex = where(viirs_cld_frac_arr[andex[ondex]]  ge 0.,nval)
                  1: vndex = where((viirs_cld_frac_arr[andex[ondex]]  ge 0.) $
                             AND (xa_type[andex[ondex]] le 2),nval)
		  2: vndex = where(viirs_cld_frac_arr[andex[ondex]]  lt 0.50,nval)
               endcase
