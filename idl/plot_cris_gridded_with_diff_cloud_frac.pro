...
if (plot_type eq 'PS') then begin
   ...
   device,filename='plot_cris_gridded_diff_cloud_frac.ps'
   !p.font = 0
endif
...
; Read VIIRS  data
   ...
   cndex = where(datav.viirs_count[0,*,*,*] lt 0.,nbad)
   if (nbad gt 0) then viirs_cld_frac[cndex] = -999.
   print, nbad
   ...
   if (ic eq 0)  then begin
      ...
      viirs_cld_frac_arr = [viirs_cld_frac_arr,reform(viirs_cld_frac,npts_cris) ]
   endelse
	 ...
	 for ilon=0,nlon-2 do begin 
	    if (na gt 0) then begin
		  ...
		  case ip of
		     ...
		     2: vndex = where(viirs_cld_frac_arr[andex[ondex]]  lt 0.25,nval)
		  endcase
