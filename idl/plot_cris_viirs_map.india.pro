...
for is=0,2 do begin
   device,filename='plot_cris_viirs_map_refl.'+viirs_subsets[is]+'.ps'
   ...
   for ic=0,ncdf_cris-1 do begin
     ...
     cndex = where(clevels gt viirs_refl[is,id_band,ic],ng)
     ;cndex = where(clevels gt viirs_refl_sdev[is,id_band,ic],ng)
     ...
     if (viirs_refl[is,id_band,ic] gt 0.0) then begin
     ;if (viirs_refl_sdev[is,id_band,ic] gt 0.0) then begin
	oplot,lon_eta,lat_eta
	polyfill,lon_eta,lat_eta,color=cores[id]
     endif
   endfor
   draw_scale_bar,'refl',[.87,.15,.92,.9],cores[0:10],clevels[0:10],['(f3.1)'],[''],[0.01,0.],charsize=0.7
   device,/close_file
endfor
